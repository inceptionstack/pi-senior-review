...
- **`/review-judge-toggle` command + status-bar indicator** — Session-level toggle for the duplicate-review suppressor (the judge). Runs without edits to `.lgtm/settings.json`; matches the pattern of `/review` (Alt+R). Status bar now shows a dim `⚖ judge` glyph whenever the judge is enabled, visible in all three status bar states (reviewing / pending files / idle).
